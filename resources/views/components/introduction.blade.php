<div id="intro">
    <x-sticky-header>
        <h1>Introduction</h1>
    </x-sticky-header>
    
    <video muted autoplay loop class="mx-auto">
        <source src="introduction.mp4" type="video/mp4">
        <source src="introduction.ogg" type="video/ogg">
        Your browser does not support the video tag.
        </video>
    <div class="p-2 text-center text-sm text-white md:w-2/3 mx-auto md:text-xl ">
        <div class="mb-4 leading-loose">
            In a world marked by increasing challenges such as climate change, 
economic uncertainties, and global health crises, EICC
in collaboration with sponsors,  partners, and affiliates emerges as a beacon of sustainable business practices. 
        </div>
        <div class="mb-4 leading-loose">
            
This comprehensive business model not only envisions profitability but, 
more importantly, addresses crucial societal needs such as food security, 
international trade, overseas employment, and humanitarian efforts.
        </div>
    </div>
</div>