...
<body>
    <div class="p-4 space-y-4">
        ...
        <h1 class="text-xl">Travel Date</h1>
        <form action="/reserve" method="post" class="space-y-4">
            <input type="date" name="date" class="border block w-full p-2 border-2 rounded-md border-green-900" required>
            <input type="hidden" name="trip_id" value="{{$trip->id}}" />
            <input type="hidden" name="user_id" value="{{$user->id}}" />
            @csrf 
            <div>
                <button type="submit" class="bg-blue-900 text-white p-4 font-bold text-lg rounded-full w-full">Submit Request</button>
            </div>
        </form>
    </div>
</body>
