...
<body class="bg-gray-100">
    <div class="p-2 space-y-4">
        ...
        <form action="/form-request/{{$user->id}}" class="flex grid grid-cols-5 gap-2">
            ...
            <a href="/form-request/{{$user->id}}" class="bg-red-700 flex items-center justify-center rounded-full text-white">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        <div class="space-y-4">
            @forelse ($record as $item)
                <div class="bg-white p-2 border rounded-xl relative space-y-2">
                    <h1 class="truncate font-bold text-xl text-gray-800">{{$item->model}}</h1>
                    ...
                    <span class="absolute top-2 right-2 text-xs text-white px-2 py-1  rounded-md {{$item->status == 'approved'  ? 'bg-green-500': 'bg-red-300'}}">{{$item->status}}</span>
                </div>
            @empty
            <div class="text-center bg-white p-4 rounded-md">No Data Found</div>
            @endforelse
        </div>
    </div>
    <x-back-home/>
