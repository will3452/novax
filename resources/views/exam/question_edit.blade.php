<x-new-layout>
    ...
    <form action="/questions/{{$question->id}}" method="POST" enctype="multipart/form-data" class="card-body">
        @csrf
        @method('PUT')
        <input type="hidden" name="exam_id" value="{{$question->exam_id}}">
        <div class="form-control" x-data="{type:{{is_null($question->question_image)}}}">
            ...
            <template x-if="type">
                <textarea required type="text" name="question" class="form-control border p-2">{{$question->question}}</textarea>
            </template>
            <template x-if="!type">
                <div>
        ...
        <div class="form-control" x-data="{type:'{{$question->type}}'}">
            <div class="label">
            ...
            <select name="type" x-model="type" class="select select-bordered w-full" required>
                ...
                @if ($question->exam->is_manual_checking)
            ...
            <div class="form-control">
                    ...
                    <div class="my-1">
                        ...
                        <div class="form-control">
                            <label class="label cursor-pointer">
                              ...
                              <input type="radio" {{$question->answer == 1 ? 'checked':''}} name="answer" value="1" class="radio checked:bg-blue-500">
                            </label>
                          </div>
                          <div class="form-control">
                            <label class="label cursor-pointer">
                              <span class="label-text">False</span>
                              <input type="radio" {{$question->answer == 0 ? 'checked':''}} name="answer" value="0" class="radio checked:bg-red-500">
                            </label>
                          </div>
                    </div>
                </template>
                <template x-if="type == `{{\App\Models\Question::TYPE_IDENTIFICATION}}`">
                    <div class="my-1">
                        ...
                        <input type="text" name="answer" value="{{$question->answer}}" required class="form-control border">
                    </div>
                </template>
                    ...
                    <div>
                        <div class="my-1">
                            ...
                            <template x-if="type == `{{\App\Models\Question::TYPE_MULTIPLE_ANSWER}}`">
                                <div>
                                    <input id="tt" type="text" name="answer" value="{{$question->answer}}" required class="input input-bordered w-full">
                                    <script>
                        ...
                        <div class="my-1">
                            ...
                            <input type="text" name="choices" value="{{$question->choices}}" required class="input input-bordered w-full">
                            <script>
        ...
        <a class="btn btn-secondary mt-4" href="{{url()->previous()}}">Cancel</a>
