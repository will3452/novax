...
<body class="bg-white md:bg-gray-200">
    <div class="h-screen grid items-center grid-cols-1 md:grid-cols-2 justify-center gap-2 md:gap-4  w-screen">
        ...
        <div class="bg-white w-full p-4 md:p-8 md:h-full rounded-xl md:rounded-none">
            <form action="{{route('nova.login')}}" method="POST" class="space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    <input name="email" required type="text" class="border w-full border-2 p-2 rounded-xl">
                </div>
                <div class="space-y-2">
                    <label for="" class="font-bold text-black block">Password</label>
                    <input name="password" required type="password" class="border w-full border-2 p-2 rounded-xl">
                </div>
                <button class="hover:bg-blue-900 hover:text-white transition w-full bg-blue-200 p-2 rounded-xl font-bold text-gray-900">
                    LOGIN
                </button>
            </form>
        </div>
