<template>
  <panel-item :field="field">
    <div slot="value" class="text-90 flex">
      <div class="w-full max-col-2 inline" v-if="field.withGroups">
      ...
      <div class="inline w-full max-col-2" v-else>
        <div
          v-for="(label, option) in field.options"
          :key="option"
          class="flex-auto"
        >
          <span class="inline-block rounded-full w-2 h-2 mr-1" :class="optionClass(option)"/>
          <span>{{ label }}</span>
        </div>
      </div>
    </div>
